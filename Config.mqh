...
private:
   ...
   void ValidateConfig()
   {
      ...
      {
         m_valid = false;
         m_error += "Invalid RR range (min > max or <=0); ";
      }
      if (m_config.ob_tolerance_pts <= 0.0)
      {
         m_valid = false;
         m_error += "OB tolerance must be >0; ";
      }
      if (ArraySize(m_config.allowed_symbols) == 0)
      {
         m_valid = false;
         m_error += "No allowed symbols specified; ";
      }
      if (m_config.max_lot_size <= 0.0)
      {
         m_valid = false;
         m_error += "Max lot size must be >0; ";
      }
      if (m_config.base_risk_pct <= 0.0 || m_config.base_risk_pct > 5.0)
      {
         m_valid = false;
         m_error += "Base risk % out of range (0-5); ";
      }
      if (m_config.max_consec_losses <= 0)
      {
         m_valid = false;
         m_error += "Max consec losses must be >0; ";
      }
      if (m_config.max_dd_pct <= 0.0 || m_config.max_dd_pct > 20.0)
      {
         m_valid = false;
         m_error += "Max DD % out of range (0-20); ";
      }
      if (m_config.sl_multiplier <= 0.0)
      {
         m_valid = false;
         m_error += "SL multiplier must be >0; ";
      }
...
public:
   ...
   double GetMinRR() const { return m_config.min_rr; }
   double GetMaxRR() const { return m_config.max_rr; }
   ENUM_TIMEFRAMES GetPrimaryTF() const { return m_config.primary_tf; }
   ENUM_TIMEFRAMES GetConfirmTF() const { return m_config.confirm_tf; }
   double GetObTolerancePts() const { return m_config.ob_tolerance_pts; }
   ...
   double GetMaxLotSize() const { return m_config.max_lot_size; }
   double GetBaseRiskPct() const { return m_config.base_risk_pct; }
   int GetMaxConsecLosses() const { return m_config.max_consec_losses; }
   double GetMaxDdPct() const { return m_config.max_dd_pct; }
   double GetSlMultiplier() const { return m_config.sl_multiplier; }
   RiskLevel GetRiskLevel() const { return m_config.risk_level; }
};
